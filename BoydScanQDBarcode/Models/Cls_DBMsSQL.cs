...
using System.Data.SqlClient; // Ensure this is present
    ...
    public bool SaveToDIWaterWashLog(string serial, string assyPN, string station, string failCode, string operatorName, string remark, string workOrder, int stepNo, int Fixture_No, float Flowrate, float Waterpressure, float Airpressure)
    {
        bool result = false;
        string query = @"
                            INSERT INTO DI_Water_Wash_Log
                            (Serial, Assy_PN, Date_Time, Station, FailCode, Operator, Remark, Work_Order, Step_No, Water_Flow_Rate, Water_Pressure, Air_Pressure, Fixture_No)
                            VALUES
                            (@Serial, @Assy_PN, @Date_Time, @Station, @FailCode, @Operator, @Remark, @Work_Order, @Step_No, @Water_Flow_Rate, @Water_Pressure, @Air_Pressure, @Fixture_No);";
        try
        {
            ...
            using (var command = new SqlCommand(query, connection))
            {
                ...
                command.Parameters.AddWithValue("@Fixture_No", Fixture_No);
    ...
    // Thêm vào đây:
    public bool SaveToDryingLog(string serial, string assyPN, string station, string failCode, string operatorName, string remark, string workOrder, 
                                int Fixture_No, double dryingtime , double airhumid, double airtemp, double airpressre, double airFlow)
    {
        bool result = false;
        string query = @"
                            INSERT INTO [Production_SZ].[dbo].[LCS_Drying_Humidity_Log]
                            ( Serial
                              ,Assy_PN
                              ,Date_Time
                              ,Station
        ...
        try
        {
            ...
            using (var command = new SqlCommand(query, connection))
            {
                ...
                command.Parameters.AddWithValue("@Station", station);
                command.Parameters.AddWithValue("@FailCode", failCode ?? "");
                command.Parameters.AddWithValue("@Drying_Time", dryingtime);
                command.Parameters.AddWithValue("@Rel_Humidity", airhumid);
                command.Parameters.AddWithValue("@Operator", operatorName ?? "");
                command.Parameters.AddWithValue("@Remark", remark ?? "");
                command.Parameters.AddWithValue("@Work_Order", workOrder ?? "");
                command.Parameters.AddWithValue("@Air_Temperature", airtemp);
                command.Parameters.AddWithValue("@Air_Pressure", airpressre);
                command.Parameters.AddWithValue("@Air_Flow", airFlow);
                command.Parameters.AddWithValue("@Fixture_No", Fixture_No);
                command.ExecuteNonQuery();
            }
            result = true;
